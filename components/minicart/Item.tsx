...
function CartItem({ item, index, locale, currency }: Props) {
    ...
    return (<fieldset 
    // deno-lint-ignore no-explicit-any
    data-item-id={(item as any).item_id} class="grid grid-rows-1 gap-2 px-4" style={{ gridTemplateColumns: "auto 1fr" }}>
      <Image alt={name} src={image.replace("-55-55", "")} style={{ aspectRatio: "80 / 80" }} width={80} height={80} class="h-full object-contain"/>

      {/* Info */}
      <div class="flex flex-col gap-2">
        {/* Name and Remove button */}
        <div class="flex justify-between items-center ">
          <legend class="text-sm font-normal text-gray-400" style={"display: block; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; text-overflow: ellipsis"}>
            {newName}
          </legend>
          <button class={clx(isGift && "hidden", "btn btn-ghost btn-square no-animation")} hx-on:click={useScript(removeItemHandler)}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M3 6h18m-2 0v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6m3 0V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2" stroke="#CF2424" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
          </button>
        </div>
