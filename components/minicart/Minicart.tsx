...
export default function Cart({ cart: { platformCart, storefront: { items, total, 
// subtotal,
coupon, discounts, locale, currency, 
// enableCoupon = true,
// freeShippingTarget,
checkoutHref, }, }, }: {
    cart: Minicart;
    ...
    return (<>
      <form class="contents" id={MINICART_FORM_ID} hx-sync="this:replace" hx-trigger="submit, change delay:300ms" hx-target="this" hx-indicator="this" hx-disabled-elt="this" hx-post={useComponent(import.meta.url)} hx-swap="outerHTML">
        ...
        <div class={clx("flex flex-col flex-grow justify-start items-center overflow-hidden w-full ", "[.htmx-request_&]:pointer-events-none [.htmx-request_&]:opacity-60 [.htmx-request_&]:cursor-wait transition-opacity duration-300")}>
          {count === 0
            ...
            : (<div class="w-full h-full flex justify-between flex-col ">
                <ul role="list" class="mt-6 px-2 flex-grow overflow-y-auto flex flex-col gap-6 w-full overflow-scroll pb-[290px] overflow-x-hidden">
                  {items.map((item, index) => (<li class="border-b border-gray-100 pb-4">
                ...
                <output form={MINICART_FORM_ID}>
                  ...
                  <div class="border-b border-gray-100 pt-4 pb-4 flex flex-col justify-end items-end gap-2 mx-4">
                    <div class="flex justify-between items-center w-full">
                      ...
                      <output form={MINICART_FORM_ID} class="text-base text-gray-400 font-bold">
                ...
                Taxas e fretes serão calculados no checkout
                  ...
                  <div class="p-4">
                    <a class="btn w-full hover:bg-orange-300 hover:border-bg-300 no-animation bg-orange-300 rounded-lg" href={checkoutHref} hx-on:click={useScript(sendBeginCheckoutEvent)}>
                      <span class="[.htmx-request_&]:hidden text-white text-base font-bold">
                        Ir para o pagamento
                      </span>
                      <span class="[.htmx-request_&]:inline hidden loading loading-spinner"/>
                    </a>
                    <a class="text-center mt-3 block text-orange-300 font-bold text-base py-4" href="javascript:void(0)" hx-on:click={useScript(onClick)}>
                      Continuar comprando
                    </a>
                  </div>
                </footer>
