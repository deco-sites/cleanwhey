...
function Section(props: SectionProps<ReturnType<typeof loader>>) {
    ...
    return (<section class={"section container md:px-0 px-4"}>
      ...
      <div class="grid md:grid-cols-3 mb-4 grid-cols-1 gap-2 md:px-0 px-4">
        {cards && cards.length > 0 &&
            cards.map((card) => (<div class="border border-gray-100 bg-white-300 rounded p-4">
              <h3 class="text-base font-bold md:text-left text-center text-orange-300">
                {card.subtitle}
              </h3>
              {card.description && (<span dangerouslySetInnerHTML={{ __html: card.description }} class="text-gray-300 text-base md:text-left text-center"/>)}
            </div>))}
      </div>
    </section>);
}
...
export const loader = (props: Props, req: Request) => {
    ...
    const section = sections?.find(({ matcher }) => new URLPattern({ pathname: matcher }).test(req.url));
    return { section };
};
