...
export default function DescriptionFullbanner(props: Props) {
    ...
    return (<section>
      <div class={`relative w-full pb-8 md:pb-16 md:mb-16 bg-no-repeat bg-center bg-cover md:min-h-96`} style={{
        ...
        <div class="w-full top-0 left-0 h-full flex flex-col md:flex-row items-center">
          <div class={`flex px-4 pt-12 md:px-0 md:items-start items-center flex-col md:flex-row h-auto gap-8 md:gap-16 container justify-end mb-96 md:mb-0`}>
            <div class="max-w-[384px]">
              ...
              {content && (<div class={"text-white [&>ul]:pl-8 [&>ul>li]:mb-2 [&>ul>li]:!list-disc"} dangerouslySetInnerHTML={{ __html: content }}/>)}
            </div>
            {rightCard && rightCard.title != "" && (<div class="bg-white rounded-lg p-4 max-w-[384px]">
                <h3 class="mb-4 text-gray-400 font-bold">{rightCard?.title}</h3>
                <hr class="mb-4 text-gray-100"/>
                {rightCard?.content && (<div class="font-normal text-gray-300 font-base [&>ul]:pl-8 [&>ul>li]:mb-2 [&>ul>li]:!list-disc" dangerouslySetInnerHTML={{ __html: rightCard?.content }}/>)}
              </div>)}
