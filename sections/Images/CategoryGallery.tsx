...
const Desktop = ({ items }: Props) => {
    return (<>
      {items &&
            items.map((item, index) => (<div class="relative group flex w-full duration-300 h-full transition-all items-center justify-center">
            <a class={`${index == 0 ? "rounded-l-lg" : ""} 
                            ${index == 3 ? "rounded-r-lg" : ""} 
                            w-full h-full transition-all duration-300 relative overflow-hidden max-h-[386px]`} href={item.link}>
              <img class={`${index == 0 ? "rounded-l-lg" : ""} 
                            ${index == 3 ? "rounded-r-lg" : ""} w-full duration-300 transition-all group-hover:scale-125 h-full max-h-[386px]`} src={item.images.normal} loading={"lazy"}/>
...
const Mobile = ({ items }: Props) => {
    const id = useId();
    return (<section id={id} class="relative">
      <Slider className="carousel carousel-center max-w-md space-x-4 p-4 md:pr-4 md:pl-4 ">
        {items &&
            items.map((item, index) => (<Slider.Item index={index} class="carousel-item relative group !ml-0 flex w-[85%] duration-300 h-full transition-all items-center justify-center">
              <a class={`${index == 0 ? "rounded-l-lg" : ""} 
                                ${index == 3 ? "rounded-r-lg" : ""} 
                                w-full h-full transition-all duration-300 relative overflow-hidden`} href={item.link}>
                <img class={`${index == 0 ? "rounded-l-lg" : ""} 
                                ${index == 3 ? "rounded-r-lg" : ""} w-full duration-300 transition-all group-hover:scale-125 h-full max-h-[370px]`} src={item.images.normal} loading={"lazy"}/>
...
export default function CategoryGallery({ title, items }: Props) {
    ...
    return (<section class="section container mt-5 md:mt-12">
      {title && (<div class={"title"}>
          <h2 class={"text-2xl  font-bold text-gray-200 text-center mb-4"} dangerouslySetInnerHTML={{ __html: title }}/>
        </div>)}
      <div className={`content ${device === "desktop" ? "grid grid-cols-4" : ""}`}>
        {device === "desktop"
            ? <Desktop items={items}/>
            : <Mobile items={items}/>}
      </div>
