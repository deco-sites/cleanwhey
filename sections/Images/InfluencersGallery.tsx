...
const Desktop = ({ items, conteudoFlutuante }: Props) => {
    return (
        <section class="grid grid-cols-4 gap-1.5 relative">
            {items && items.map((item, index) => (
                <div class="relative group flex w-full duration-300 h-full transition-all items-center justify-center">
                    <a class={`rounded-lg w-full h-full transition-all duration-300 relative overflow-hidden`}
                        href={item.link} >
                        <img
            ...
            <div class="absolute m-auto left-0 right-0 z-30 bottom-[-8rem] max-w-md border rounded-lg p-6 border-gray-200 bg-white">
    ...
    return (
        <section id={id} class="relative">
            <Slider className="carousel carousel-center max-w-md space-x-4 p-4 gap-1.5 md:pr-4 md:pl-4 pr-0">
                {items && items.map((item, index) => (
                    <Slider.Item index={index} class="carousel-item relative group !ml-0 flex w-[85%] duration-300 h-full transition-all items-center justify-center">
                        <a class={`rounded-lg w-full h-full transition-all duration-300 relative overflow-hidden`}
                            href={item.link} >
                            <img
                                class={`rounded-lg w-full duration-300 transition-all group-hover:blur`}
            ...
            <div class="absolute m-auto left-0 right-0 z-30 bottom-[-12rem] !text-center max-w-[90%] border rounded-lg p-6 border-gray-200 bg-white">
                <div class="text-gray-300 mb-4" dangerouslySetInnerHTML={{__html: conteudoFlutuante.text}}  />
                <a href={conteudoFlutuante.link} class="bg-orange-300 block text-center w-full rounded-lg py-2 px-4 text-white text-base font-normal">
                    {conteudoFlutuante.label ? conteudoFlutuante.label : "Saiba mais"}
                </a>
