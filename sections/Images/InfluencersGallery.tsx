    ...
    <section>
      {items && items.length <= 4 && (
        <div class="grid grid-cols-4 gap-1.5 relative">
          {items.map((item, index) => (<div key={index} class="relative group flex w-full duration-300 h-full transition-all items-center justify-center overflow-y-auto ">
                <a class={`rounded-lg w-full h-full transition-all duration-300 relative `} href={item.link}>
                  <Image class={`rounded-lg w-full duration-300 transition-all  min-h-[470px]  max-h-[470px] h-full object-fill group-hover:blur`} src={item.images.normal} loading={"lazy"} width={300} height={470}/>
                  ...
                  <div class="font-regular text-sm overflow-y-auto leading-4 transition-all text-left text-white py-8 px-6 duration-300 hidden bg-[#385A88CC] group-hover:flex flex-col w-full h-full z-10 absolute top-0 left-0 items-start justify-start scroll-custom overflow-x-hidden rounded-md">
                    ...
                    <span dangerouslySetInnerHTML={{ __html: item.description }} class="[&_*]:text-[14x] [&_*]:leading-normal"/>
                  </div>
                </a>
        ...
        <Slider className="carousel gap-1.5 md:pr-4 md:pl-4">
          {items &&
            ...
            class="carousel-item relative group !ml-0 flex w-1/4 duration-300 h-full transition-all items-center justify-center">
                <a class={`rounded-lg w-full h-full transition-all duration-300 relative overflow-hidden`} href={item.link}>
                  <Image class={`rounded-lg w-full duration-300 transition-all  min-h-[470px] max-h-[470px] h-full `} src={item.images.normal} loading={"lazy"} width={300} height={470}/>
      ...
      <div class="relative mt-2.5 m-auto z-30 !text-center max-w-lg border rounded-lg p-6 border-gray-200 bg-white">
        <div class="text-gray-300 mb-4" dangerouslySetInnerHTML={{ __html: conteudoFlutuante.text }}/>
        <a href={conteudoFlutuante.link} class="bg-orange-300 block text-center w-full rounded-lg py-2 px-4 text-white text-base font-normal">
    ...
    return (<section>
      <div id={id} class="relative">
        <Slider className="carousel carousel-center max-w-md space-x-4 p-4 gap-1.5 md:pr-4 md:pl-4 pr-0">
          {items &&
            items.map((item, index) => (<Slider.Item index={index} class="carousel-item relative group !ml-0 flex w-[85%] duration-300 h-full transition-all items-center justify-center">
                <a class={`rounded-lg w-full h-full transition-all duration-300 relative overflow-hidden`} href={item.link}>
                  <Image class={`rounded-lg w-full duration-300 transition-all  min-h-[470px] max-h-[470px] h-full `} src={item.images.normal} loading={"lazy"} width={300} height={470}/>
                  <div class="font-regular text-sm leading-4 transition-all text-left text-white py-8 px-6 duration-300 hidden bg-[#385A88CC] group-hover:flex flex-col w-full h-full z-10 absolute top-0 left-0 overflow-y-scroll items-start justify-start scroll-custom overflow-x-hidden rounded-md">
                    <h2 class={"font-bold mb-6"}>{item.textLabel}</h2>
...
export default function InfluencersGallery({ title, items, conteudoFlutuante, }: Props) {
    ...
    return (<section class="section container mb-12">
      {title && (<div class={"title"}>
          <h2 class={"text-2xl font-bold text-gray-200 text-center mb-4 px-4"} dangerouslySetInnerHTML={{ __html: title }}/>
        </div>)}
