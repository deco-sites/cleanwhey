...
const Desktop = ({ items }: Props) => {
    return (<>
      {items &&
            items.map((item) => (<div class="relative flex w-full flex-col items-center justify-center">
            <a class={`w-full h-full relative overflow-hidden`} href={item.link?.url}>
              <img class={`w-full h-full`} src={item.image} width={455} height={428} loading={"lazy"}/>
            </a>
            <div class="text-white drop-shadow-[0_0_15px_rgba(0,0,0,0.50)] absolute top-0  left-0 w-full h-full flex flex-col items-center justify-center text-center">
              ...
              <h2 class={`max-w-[275px] ${item.flag ? "mb-6" : "mb-12"} font-bold text-xl md:text-2xl text-white`}>
                {item.title}
              </h2>
              {item.flag && <img class="max-w-[150px]" src={item.flag}/>}
              {!item.flag && (<span class="text-white text-sm font-regular">
                  {item.link?.title}
                </span>)}
            </div>
          </div>))}
...
export default function TripleGallery({ items }: Props) {
    ...
    return (<section class="section">
      <div className={`content ${device === "desktop" ? "grid grid-cols-3" : ""}`}>
        {device === "desktop" ? (<Desktop items={items}/>) : (<Mobile items={items}/>)}
      </div>
    </section>);
}
