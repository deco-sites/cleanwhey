...
function Notice({ title, description, icon }: {
    ...
    return (<div class="flex flex-col gap-3 justify-start items-start sm:items-start">
    ...
    return (<section class="bg-orange-300">
      <Section.Container class="border-b border-b-orange-200">
        <div class="px-4 sm:px-14 py-4 flex flex-col sm:flex-row items-center justify-start sm:justify-center gap-8">
          ...
          <form hx-target="closest section" hx-swap="outerHTML" hx-post={useComponent(import.meta.url)} class="flex flex-col sm:flex-row gap-4 w-full max-w-96">
            <input name="email" class="input input-bordered rounded-lg flex-grow" type="text" placeholder={placeholder}/>

            <button class="btn btn-primary rounded-lg bg-blue-300 border-blue-300" type="submit">
