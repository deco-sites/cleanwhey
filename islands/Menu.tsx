...
function MenuItem({ item }: { item: SiteNavigationElement }) {
  ...
  const title = (
    <p class={`py-[20px] pl-[30px] pr-[12px] tracking-[0.7px] text-base font-bold text-gray-300 flex-grow min-h-[40px] flex items-center justify-start`}>
      {item.name}
    </p>
  )
  ...
  return (
    <li class="last:border-b!">
      <div class={`flex justify-between items-center w-full border-b border-gray-100`}
      onClick={() => {if(hasChildren) open.value = !open.value; console.log(open.value)}}
      >
        {hasChildren ? title : <a class="w-full inline-block text-base font-bold text-gray-300" href={item.url}>{title}</a>}

        {hasChildren && (
          <button class="mr-4">
            <Icon class={"block text-orange-300"}
              id="chevron-right"
              height={24}
              width={24}
            />
        ...
        {hasChildren && (
          <ul class={`${open.value ? "right-0 visible" : "right-[-100vw] invisible"} top-[140px] h-[calc(100%-138px)]
          ...
          `}>
            ...
            <li class="w-full relative flex justify-start items-center h-[60px] border-b border-gray-100">
             ...
             <p href={item.url} class="w-full h-[60px] py-2 ml-10 flex justify-start items-center text-left">
            ...
            <li>
              <a href={item.url} class="py-[20px] pl-[30px] pr-[12px] flex-grow min-h-[40px] flex items-center justify-start">
